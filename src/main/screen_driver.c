...
void refresh_display(void)
{
    ...
    uint8_t height= 64;
	uint8_t width=132; //2 pixel overrun to either side 
	uint8_t m_row = 0;
	uint8_t m_col = 2;
...
void static draw_byte(uint8_t x, uint8_t y, uint8_t b, uint8_t type)
{
    if( Display_Buffer_Mutex != NULL )
    {
        if( xSemaphoreTake( Display_Buffer_Mutex, ( TickType_t ) 10 ) == pdTRUE )
        {  
            ...
            uint16_t buffer_index = y / 8 * SH1106_LCDWIDTH + x;
...
void static draw_bytes(uint8_t x, uint8_t y, const uint8_t* data, uint8_t size, uint8_t scaling, uint8_t type)
{
...
void static draw_character(uint8_t x, uint8_t y, char c, uint8_t scaling, uint8_t type)
{
    // Invalid position
